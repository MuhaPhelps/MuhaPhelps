...
class BookFormScreen extends StatefulWidget {
  ...
  @override
  State<BookFormScreen> createState() =>
...
class _BookFormScreenState
  ...
  Future<void> _loadInitialData() async {
    final lookupCache =
        context.read<LookupCache>();

    final bookNotifier =
        context.read<BookListNotifier>();

    // Запускаем три загрузки сразу.
    // При первом открытии формы они пойдут
    // на сервер. При следующих открытиях
    // LookupCache вернёт уже сохранённые
    // результаты.
    final authorsFuture =
        lookupCache.getAuthors();

    final genresFuture =
        lookupCache.getGenres();

    final publishersFuture =
        lookupCache.getPublishers();
